...
- [[#basic-configuration][Basic Configuration]]
  - [[#-auto-shutdown-emacs-daemon-after-inactivity][💤 Auto-shutdown Emacs daemon after inactivity]]
...
** 💤 Auto-shutdown Emacs daemon after inactivity
When running Emacs as a daemon, it can be nice to have it automatically exit
after a period of inactivity — for example, when all client frames are closed
and no buffers are being used.

This block defines a timer that checks periodically whether:
- Emacs is running as a daemon (`daemonp`)
- There are no open frames (`frame-list`)
- There are no connected clients (`server-clients`)
If all are true for a given time interval (e.g. 10 minutes), Emacs will shut down cleanly.
